...
// 1. Inject getApiFuncs inside renderListManager
if (!content.includes('const getApiFuncs = () => {')) {
...
// 2. Replace handleSaveItem
const saveItemRegex = /const handleSaveItem = \(\) => \{[\s\S]*?setDraftItem\(\{\}\);\s*\};/;
const newHandleSaveItem = `const handleSaveItem = async () => {
      let nextList = [];
      try {
        const funcs = typeof getApiFuncs === 'function' ? getApiFuncs() : null;
        let savedItem;
...
// 3. Replace deleteConfirm
const deleteConfirmRegex = /onConfirm=\{\(\) => \{[\s\S]*?showToast\("Record deleted successfully\."\);\s*\}\}/;
...
console.log('Fixed handleSaveItem and deleteConfirm!');
