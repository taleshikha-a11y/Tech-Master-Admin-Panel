...
export const Contact = () => {
  ...
  const handleSaveSettings = (section, draft, msg) => {
    ...
    try { fetch(`${import.meta.env.VITE_API_URL}/api/contact/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(draft) }); } catch(err){}
    showToast(msg);
  };
  ...
  const handleSaveList = (e, type) => {
    ...
    if (type === 'field') {
      ...
      if (editingField) {
        ...
        try { fetch(`${import.meta.env.VITE_API_URL}/api/contact/${editingField.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(fieldForm) }); } catch(err){}
      } else {
        updateSection('contactFormFields', null, [...list, { ...fieldForm, id: `cff-${Date.now()}` }]);
        try { fetch(`${import.meta.env.VITE_API_URL}/api/contact/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(fieldForm) }); } catch(err){}
      }
      ...
      if (editingCat) {
        ...
        try { fetch(`${import.meta.env.VITE_API_URL}/api/contact/${editingCat.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(catForm) }); } catch(err){}
      } else {
        updateSection('contactCategoriesSetup', null, [...list, { ...catForm, id: `cc-${Date.now()}` }]);
        try { fetch(`${import.meta.env.VITE_API_URL}/api/contact/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(catForm) }); } catch(err){}
      }
      ...
      if (editingSocial) {
        ...
        try { fetch(`${import.meta.env.VITE_API_URL}/api/contact/${editingSocial.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(socialForm) }); } catch(err){}
      } else {
        updateSection('contactSocialLinksSetup', null, [...list, { ...socialForm, id: `csl-${Date.now()}` }]);
        try { fetch(`${import.meta.env.VITE_API_URL}/api/contact/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(socialForm) }); } catch(err){}
      }
